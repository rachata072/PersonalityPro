...
const MBTIQuiz = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        setLoading(true);
        const fetchedQuestions = await getQuestions('mbti');
        console.log('Fetched questions:', fetchedQuestions.length);
        setQuestions(fetchedQuestions as MBTIQuestion[]);
        setError(null);
      } catch (error) {
  ...
  const handleAnswer = async (answer: number) => {
    ...
    if (currentQuestionIndex < questions.length - 1) {
      ...
      try {
        ...
        const result = await submitAnswers('mbti', newAnswers);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <button
          onClick={handleBack}
          className="text-gray-600 hover:text-purple-600 flex items-center transition-colors"
        >
